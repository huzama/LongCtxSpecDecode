...
Pointers: [DrafterGoesBurrrr.md](DrafterGoesBurrrr.md) for the method and its results, [handoff.md](handoff.md) for what exists in the code and what was found building it, [literature.yaml](literature.yaml) for the survey, branch `survey-and-prototypes` for everything before the fork.
...
- [x] Baseline grid: dense and vegas at 32K-128K, batch 1 to saturation, on a quiet node; vegas acceptance reproduces its paper. Numbers in DrafterGoesBurrrr.md.
- [x] Method: longspec drafting designed and implemented as `longspec`, unit tests and parity test green. Design in DrafterGoesBurrrr.md.
- [x] Grid T2-T4: coverage (selection alone) at θ 0.90/0.95/0.98 vs dense and vegas on the five cells. Numbers in DrafterGoesBurrrr.md.
- [x] Parity gate: `test_parity` (θ 1, uncapped) reproduces dense output token for token; `grid.py --parity` checks any cell.
- [x] Packed verify attention: GQA group packed into rows for the verify's prefix, causal tail, LSE merge. +16% at 32K b4, acceptance neutral, lossless; b1 and 64K b2 flat, blocked on splits (to-do 1). Numbers in DrafterGoesBurrrr.md.
...
- [x] W4A16 draft weights, stage A control: agreement 0.92-0.93, Marlin 1.66x at b1, no dequant crossover through b4, AWQ = GPTQ at 4B. Numbers in DrafterGoesBurrrr.md.
- [x] W4A16 draft weights, stage B: `sparse_attn_draft_weights` drafts on a quantized copy sharing the target's attention, embeddings and lm_head. 60.2 tok/s at 32K b1 (1.23x dense), 156.0 at 32K b4 (1.66x dense). Numbers in DrafterGoesBurrrr.md.
...
- [x] Profile of the round at 32K b1, 32K b4, 64K b1: the verify attention reads the KV four times at 38% occupancy (FA2 packs GQA only at `seqlen_q == 1`); the draft step sits on the 8 GB weight floor; launch overhead is second order. Numbers in DrafterGoesBurrrr.md.
- [x] Matched bytes vs vegas, both on W4, coverage tuned to a ~7% mean budget: coverage throughput equal or ahead in all four cells; alpha ahead at b1 both contexts, behind at 32K b4, tied at 64K b2. θ near 0.92 is a flat, better operating point than 0.98 under W4. Numbers in DrafterGoesBurrrr.md.
...
- [ ] 3. 128K b1 coverage below dense (18.9 at θ 0.926, 21.8 at θ 0.98, dense 23.5): memory starvation ruled out from the run logs (zero preemptions, pool at 21% peak); the verify-occupancy wall (to-do 2) is the standing suspect. Confirm by profiling a 128K round after any occupancy fix.
- [ ] 4. Quant loss at long context: the 4-bit copy's agreement falls from 0.93 at 32K to 0.85 at 64K (bf16 self 0.98), so the 64K deficit is quantization, not selection. Decide 4-bit against 8-bit draft weights for contexts of 64K and above; an 8-bit copy is one config value.
